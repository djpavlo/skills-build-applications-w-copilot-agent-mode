...
export default function Activities() {
  ...
  return (
    <div className="card">
      <div className="card-body">
        ...
        <div className="table-responsive">
          <table className="table table-striped table-bordered">
            <thead>
              <tr>
                {columns.length ? columns.map((c) => <th key={c}>{c}</th>) : <th>No data</th>}
              </tr>
            </thead>
            <tbody>
              {items && items.length ? (
                items.map((row, idx) => (
                  <tr key={idx} style={{cursor: 'pointer'}} onClick={() => { setRawData(row); setShowRaw(true); }}>
                    {columns.map((c) => (
                      <td key={c}>{String(row[c] === undefined || row[c] === null ? '' : row[c])}</td>
                    ))}
                  </tr>
                ))
              ) : (
                <tr><td colSpan={columns.length || 1}>{loading ? 'Loading...' : 'No activities found'}</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <SimpleModal show={showRaw} onClose={() => setShowRaw(false)} title="Activities JSON">
